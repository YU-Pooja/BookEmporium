...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="z-50 relative flex bg-white text-zinc-800 px-8 py-1 items-center justify-between">
        <div className="flex items-center justify-between">
          {/* <img className="h-10 me-4" src="{logoo}" alt="logo" /> */}
          <LiaBookReaderSolid className='w-20 h-20'/>
        ...
        <div className='nav-links-bookheaven block flex gap-2 items-center'>
          <div className='hidden md:flex gap-4'>
            {links.map((items, index) => (
              <Link to={items.link} className="hover:text-blue-500 transition-all duration-300 text-2l font-semibold" key={index}>
                {items.title}{" "}
              </Link>
            ))}
          </div>
          <div className='hidden md:flex gap-2'>
            <Link to="/LogIn" className='font-semibold px-4 py-1 border rounded border-blue-500 hover:bg-blue-500 hover:text-zinc-800 transition-all duration-300'>LogIn</Link>
            <Link to="/SignUp" className='font-semibold px-4 py-1 rounded bg-blue-500 hover:bg-white hover:text-zinc-800 transition-all duration-300'>SignUp</Link>
          </div>
